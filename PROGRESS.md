...
| Tailwind config | ✅ | — | packages/ui/src/globals.css (CSS-first @theme inline) | Migrated to Tailwind 4. tailwind.config.ts deleted. All tokens declared in @theme inline blocks in globals.css per package/app. @tailwindcss/postcss replaces autoprefixer. cn() utility added at packages/ui/src/lib/generic/cn.ts. Updated 2026-05-20. Updated 2026-05-27: @font-face rules added for Futura PT (Book 400, Medium 500, Bold 700, Heavy 900). Font files in each app's public/fonts/futura/. --font-futura Tailwind theme token added. All font-['Futura_PT'] classes replaced with font-futura. |
| AgeModal | ✅ | node 1048:19703 | packages/ui/src/components/age-modal/age-modal.tsx | Age verification modal shown on first visit. Fixed scrim (bg-black/50), not dismissible by clicking outside. bg-outline card, 390px max-width, rounded-lg. Heading: text-[36px] bold text-on-surface-light centered. Two CTAs: "I am over 18" (variant="tertiary", full width) + "Leave Site" (text-primary underlined). Persists via localStorage (site-specific key). Added to SSM, SFB, SFSG layouts. Added 2026-05-27. |
