...
| Signup landing page | ✅ | node 593:4637 | apps/ssm/src/app/signup/page.tsx | Mobile-only for now (desktop design TBD). Route: /signup. Structure: SsmNav → USP → 192px hero image placeholder (bg-[#5d5d5d]) → TopTCs instruction → form section (Email + Phone TextFields, ConsentForm folded, Sign Me Up primary button, Skip For Now text button) → TopTCs legal disclaimer → SsmFooter. Both buttons navigate to /. Desktop layout to be added when Figma is provided. |
