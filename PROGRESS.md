...
| AgeModal | ✅ | node 1048:19703 | packages/ui/src/components/age-modal/age-modal.tsx | Age verification modal shown on first visit. Fixed scrim (bg-black/50), not dismissible by clicking outside. bg-outline card, 390px max-width, rounded-lg. Heading: text-[36px] bold text-on-surface-light centered. Two CTAs: "I am over 18" (variant="tertiary", full width) + "Leave Site" (text-on-surface-light underlined — was text-primary which turned green on SFB). Persists via localStorage (site-specific key). Added to SSM, SFB, SFSG layouts. Added 2026-05-27. |
...
| SfbetsFooter | ✅ | node 962:6297 (NJ variant) | packages/ui/src/components/sfbets-footer/sfbets-footer.tsx | NJ state footer. 4 nav links. 1 row of 4 NJ logos (21+, 1-800-GAMBLER, rg-nj, NJ DGE seal) in 4-col desktop / 2-col mobile grid, each with description below. Multi-state helpline legal text + NJ copyright. Logo files at /sfbets/footer/ (rg-nj.png, nj-dge.png). Figma note: PA + WV copyright lines say "MI" — fix before building those states. |
