...
|---|---|---|---|---|
| Token system | ✅ | LSM-foundation (ontario + SSM modes) | packages/tokens/index.css | Color vars per site, spacing + radius shared. primary-text added for text CTA colour; SSM value #1D8E1D. |
...
| OfferCard | ✅ | node 549:11989 | packages/ui/src/components/OfferCard/OfferCard.tsx | 2 responsive variants (mobile/desktop); uses Label + Button. Mobile: logo LEFT (144×72 container) and offer text RIGHT; desktop: logo LEFT (224×112 actual-logo container), offer+checkmarks MIDDLE, button group RIGHT (256px). Real logo fills available container height with proportional width. Both variants: PLAY NOW (primary) + optional underlined "How To Claim" text button using primary-text. Props: secondaryCtaText + secondaryCtaHref. Updated 2026-05-06. |
...
| LogoSection | ✅ | node 583:12161 | packages/ui/src/components/LogoSection/LogoSection.tsx | Responsive header bar. Mobile (40px): invisible spacer left, SSMLogo 45×45 center, tertiary Menu icon right. Desktop (72px): Menu icon + "Menu" label (surface-inverse-new) left, SSMLogo 84×84 center, invisible spacer right. Layout swap via Tailwind order classes. Inner wrapper max-w-[1440px] mx-auto constrains content on wide screens. Lego_Deco.png decorative background. onMenuClick + src props; Storybook story included. Updated 2026-05-08. |
...
| How To Claim page | ✅ | node 584:4036 (mobile) + node 583:14784 (desktop) | apps/ssm/src/app/how-to-claim/[slug]/page.tsx | Dynamic route per casino slug. Mobile: vertical operator card (logo 144×77, headline 36px, trust badges centred, steps 14px, full-width CTA) → review body text + CTA + WebsiteDirectory. Desktop: 3-col operator card (headline-block flex-1 | steps flex-1 | CTA auto, p-16 gap-8, logo 224×120, headline 45px, steps 16px) → structured article (px-16, bold intro + 2-col section grid gap-8, half-width CTA) → directory + SignupForm side-by-side (px-16 py-8). Casino data in apps/ssm/src/data/casinos.ts — includes reviewIntro + reviewSections[] for desktop. |
...
| About Us page | ✅ | node 641:12740 (desktop) + 641:12747 (mobile) | apps/ssm/src/app/om-os/page.tsx | Route: /om-os. SsmNav → USP → constrained body section (px-4 py-8 mobile / px-16 py-12 desktop) → SsmFooter. Title "About Us 📑" in text-tertiary (32px mobile / 45px desktop). Multi-paragraph Danish body copy (whitespace-pre-line, tracking-[0.5px], on-surface-light). Uses shared legalText from site-content.ts. |
| About Us page | ✅ | node 641:12747 (mobile) + node 641:12740 (desktop) | apps/ssm/src/app/om-os/page.tsx | Route: /om-os. SsmNav → USP → About text section → SsmFooter. Mobile: px-4 py-12, 32px title, 16px/24px body. Desktop: max-w-[1440px] mx-auto px-16 py-12, 45px title, body constrained to 948px to match Figma. |
