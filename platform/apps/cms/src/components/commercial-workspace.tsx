...
import { BadgePoundSterling, BriefcaseBusiness, Plus, Save, Trash2 } from 'lucide-react';
...
export function CommercialWorkspace({
    ...
    const [accountDrafts, setAccountDrafts] = useState<Record<string, CmsBrandAccountDetails>>(
        Object.fromEntries(accounts.map((account) => [account.id, accountDetails(account)]))
    );
    const [dealDrafts, setDealDrafts] = useState<Record<string, CmsCommercialDealDetails>>(
        Object.fromEntries(deals.map((deal) => [deal.id, dealDetails(deal)]))
    );
    const activeDealCount = deals.filter((deal) => getOfferScheduleStatus(deal) === 'live').length;
    const inputClass =
        ...
        <section className="flex flex-col gap-4">
            <div className="rounded-xl border border-m3-outline-variant bg-m3-surface-lowest p-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0 flex-1">
                    ...
                    <h2 className="mt-1 text-[20px] font-medium">Brand accounts and deals</h2>
                ...
                <div className="flex shrink-0 items-center gap-2">
                    <div className="rounded-lg border border-m3-outline-variant bg-m3-surface-low px-3 py-2 text-center">
                        ...
                        <div className="text-[10px] text-m3-on-surface-variant">Accounts</div>
                    </div>
                    <div className="rounded-lg border border-m3-outline-variant bg-m3-surface-low px-3 py-2 text-center">
                        <div className="text-[18px] font-medium text-m3-success">
                            {activeDealCount}
                        </div>
                        <div className="text-[10px] text-m3-on-surface-variant">Active deals</div>
                    </div>
                    <button
                        type="button"
            ...
            {accounts.map((account) => {
                const draft = accountDrafts[account.id] ?? accountDetails(account);
                const accountDeals = deals.filter((deal) => deal.brandAccountId === account.id);
                return (
                    <article
                        key={account.id}
                    ...
                    >
                        ...
                        <div className="p-4 flex flex-col gap-5">
                            ...
                            <div className="flex items-center justify-between border-t border-m3-outline-variant pt-4">
                                <div>
                                    <div className="text-[12px] font-medium">Commercial deals</div>
            ...
            {accounts.length === 0 && (
                <div className="rounded-xl border border-dashed border-m3-outline-variant bg-m3-surface-lowest p-10 text-center">
                    <BriefcaseBusiness size={24} className="mx-auto text-m3-on-surface-variant" />
                    <div className="mt-3 text-[14px] font-medium">No brand accounts yet</div>
                    <p className="mt-1 text-[11px] text-m3-on-surface-variant">
                        Add the operator’s affiliate-network account, then record its commercial
                        deals.
                    </p>
                </div>
            )}
