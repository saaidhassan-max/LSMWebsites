...
export function CampaignTimeline({ campaigns }: CampaignTimelineProps): React.ReactElement {
    ...
    const endIso = isoDay(new Date(visibleMonth.getFullYear(), visibleMonth.getMonth(), days));
    const todayIso = isoDay(new Date());
    const gridStyle = {
        gridTemplateColumns: 'minmax(150px, 190px) repeat(' + days + ', minmax(28px, 1fr))'
    };
        ...
        <section className="rounded-xl border border-m3-outline-variant bg-m3-surface-lowest overflow-hidden">
            ...
            <div className="overflow-x-auto p-4">
                <div className="min-w-[1040px]">
                    <div className="grid border-b border-m3-outline-variant" style={gridStyle}>
                        <div className="p-2 text-[10px] font-medium uppercase tracking-wide text-m3-on-surface-variant">
                            Campaign
                        </div>
                        {Array.from({ length: days }, (_, index) => {
                            const day = index + 1;
                            const dateIso = startIso.slice(0, 8) + String(day).padStart(2, '0');
                            return (
                                <div
                                    key={day}
                                    className={
