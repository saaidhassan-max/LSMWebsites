...
import type { CmsBrandAccount, CmsCommercialDeal } from '@/lib/commercial.types';
...
function scheduleSummary(item: { startDate: string | null; endDate: string | null }): string {
...
export function OperatorEditor({
    ...
    const [campaignDrafts, setCampaignDrafts] = useState<Record<string, CmsCampaignDetails>>(
        Object.fromEntries(campaigns.map((campaign) => [campaign.id, campaignDetails(campaign)]))
    );
    ...
    const visibleLandingPages = relevantLandingPages.filter((page) => {
        const matchesType = creativeFilter === 'all' || creativeFilter === 'landingPages';
        const pageStatus: OfferScheduleStatus = page.status === 'published' ? 'live' : 'hidden';
        const matchesStatus = statusFilter === 'all' || statusFilter === pageStatus;
        const matchesQuery = lowerQuery === '' || page.name.toLowerCase().includes(lowerQuery);
        return matchesType && matchesStatus && matchesQuery;
    });
    ...
    function placementChips(offerId: string): React.ReactElement {
        ...
        if (rows.length === 0) {
            return <span className="text-[11px] text-m3-on-surface-variant">Not placed</span>;
        }
        return (
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="text-[12px] text-m3-on-surface-variant mt-1">
                                            Offers, banners and campaign landing pages with
                                            schedule, placement and tracking context.
                                        </p>
                                    </div>
                                    <button
                                        type="button"
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center justify-end gap-1 border-t border-m3-outline-variant pt-2 lg:border-t-0 lg:pt-0">
                                                    <button
                                                        type="button"
                                                        onClick={() =>
                                                            removeOfferPlacements(offer.id)
                                                        }
                                ...
                                <div className="rounded-xl border border-m3-outline-variant bg-m3-surface-lowest p-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="mt-1 w-full text-[12px] text-m3-on-surface-variant">
                                            ...
                                            dates to its offers; activate or pause sets the
                                            visibility intent for all selected creative before the
                                            next global Publish.
                                        </p>
                                    </div>
                                    <button
                                        ...
                                        {campaigns.length === 0 && (
                                            <div className="rounded-xl border border-dashed border-m3-outline-variant bg-m3-surface-lowest p-10 text-center">
                                                ...
                                                <p className="text-[11px] text-m3-on-surface-variant mt-1">
                                                    Create one to group this operator’s offer,
