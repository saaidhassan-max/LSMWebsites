...
export async function listBrandAccounts(operatorId: string): Promise<CmsBrandAccount[]> {
...
export async function listCommercialDeals(accountIds: string[]): Promise<CmsCommercialDeal[]> {
    const doc = await readAll();
...
export async function setBrandAccountStatus(id: string, status: CmsRecordStatus): Promise<void> {
    const doc = await readAll();
    const accounts = doc.accounts.map((account) =>
...
export async function createCommercialDeal(brandAccountId: string): Promise<string> {
    const doc = await readAll();
...
export async function setCommercialDealStatus(id: string, status: CmsRecordStatus): Promise<void> {
    const doc = await readAll();
    const deals = doc.deals.map((deal) =>
