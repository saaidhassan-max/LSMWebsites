import type { CmsCampaign, CmsCampaignDetails, CmsRecordStatus } from './cms-content.types';
...
function normalizeCampaign(campaign: CmsCampaign): CmsCampaign {
    return {
        ...
        landingPageIds: Array.isArray(campaign.landingPageIds) ? campaign.landingPageIds : [],
...
export async function getCampaign(id: string): Promise<CmsCampaign | undefined> {
    const campaigns = await readAll();
...
export async function updateCampaign(id: string, details: CmsCampaignDetails): Promise<void> {
    const campaigns = await readAll();
...
export async function setCampaignStatus(id: string, status: CmsRecordStatus): Promise<void> {
    const campaigns = await readAll();
    const next = campaigns.map((campaign) =>
