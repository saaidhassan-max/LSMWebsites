...
function collectPlacements(
    ...
    const add = (offerId: string, label: string): void => {
        ...
        if (!current.some((placement) => placement.label === label && placement.state === state)) {
            current.push({ label, state });
        }
        placements[offerId] = current;
    };
    const addFromSections = (sections: SitePageSection[], label: string): void => {
        sections.forEach((section) => {
            if (section.type !== 'offers') return;
            section.content.items.forEach((item) => {
                if (item.kind === 'offer' || item.tie === 'offer') add(item.offerId, label);
            });
        });
    };
    addFromSections(home?.sections ?? [], 'Home');
    (pages ?? []).forEach((page) => addFromSections(page?.sections ?? [], page.name));
    return placements;
}
...
function mergePlacements(
    ...
    return [...ids].reduce<Record<string, CmsCreativePlacement[]>>((result, id) => {
        result[id] = [...(live[id] ?? []), ...(draft[id] ?? [])].filter(
            (placement, index, rows) =>
                rows.findIndex(
                    (row) => row.label === placement.label && row.state === placement.state
                ) === index
        );
        return result;
    }, {});
...
export default async function EditOperatorScreen({
    ...
    const [operator, offers, home, pages, published, campaigns, landingPages, brandAccounts] =
        await Promise.all([
            getOperator(id),
            listOffers(),
            getHomeConfig(),
    ...
    const offerUsage = operatorOffers.reduce<Record<string, 'live' | 'draft' | 'notPlaced'>>(
        (acc, offer) => {
    ...
    const commercialDeals = await listCommercialDeals(brandAccounts.map((account) => account.id));
    return (
        <OperatorEditor
            operator={operator}
