...
export async function removeOfferPlacementsAction(
  id: string,
  operatorId?: string,
): Promise<void> {
  await removeHomeOfferIds([id]);
...
export async function saveOperatorLogoAction(
  id: string,
  logoSrc: string,
): Promise<void> {
...
export async function setCampaignStatusAction(
  ...
  await Promise.all([
    ...
    ...campaign.landingPageIds.map((pageId) =>
      setPublished(pageId, status === "active"),
    ),
  ]);
  revalidatePath("/operators/edit/" + operatorId);
  revalidatePath("/offers");
