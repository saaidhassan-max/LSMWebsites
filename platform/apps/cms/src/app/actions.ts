...
import type { CmsBrandAccountDetails, CmsCommercialDealDetails } from '@/lib/commercial.types';
...
export async function createOfferForOperatorAction(
    operatorId: string,
    returnTo?: string
): Promise<void> {
...
export async function createCampaignAction(operatorId: string): Promise<string> {
    const id = await createCampaign(operatorId);
    revalidatePath('/operators/edit/' + operatorId);
    return id;
}
...
export async function saveCampaignAction(
    ...
    await updateCampaign(id, details);
    await Promise.all(
        details.offerIds.map(async (offerId) => {
            const offer = await getOffer(offerId);
            if (offer === undefined || offer.operatorId !== operatorId) return;
...
export async function setCampaignStatusAction(
    ...
    if (campaign === undefined || campaign.operatorId !== operatorId) return;
    await setCampaignStatus(id, status);
    await Promise.all([
        ...campaign.offerIds.map((offerId) => setOfferStatus(offerId, status)),
        ...campaign.landingPageIds.map((pageId) => setPublished(pageId, status === 'active'))
    ]);
    revalidatePath('/operators/edit/' + operatorId);
    revalidatePath('/offers');
    revalidatePath('/');
...
export async function deleteCampaignAction(id: string, operatorId: string): Promise<void> {
    await deleteCampaign(id);
    revalidatePath('/operators/edit/' + operatorId);
}

export async function createBrandAccountAction(operatorId: string): Promise<string> {
...
export async function deleteBrandAccountAction(id: string, operatorId: string): Promise<void> {
...
export async function deleteCommercialDealAction(id: string, operatorId: string): Promise<void> {
